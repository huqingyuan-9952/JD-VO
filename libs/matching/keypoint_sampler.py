...
class KeypointSampler():
    ...
    def km_selection(self, cur_data, ref_data,kp1):
        ...
        kp1_dense = image_grid(h, w)
        kp1_dense = np.expand_dims(kp1_dense, 0)
        tmp_flow_data = np.transpose(np.expand_dims(ref_data['flow'], 0), (0, 2, 3, 1))
        kp2_dense = kp1_dense + tmp_flow_data
        ...
        tmp_flow = np.zeros((1,24,80,1), dtype=np.float32)
        for i in range(kp1.shape[1]):  # 24
            for j in range(kp1.shape[2]):  # 80
                ...
                if 0 <= x < tmp_flow_data.shape[2] and 0 <= y < tmp_flow_data.shape[1]:
                    ...
                    tmp_flow[0, i, j] = np.linalg.norm(flow_value)

        """ best-N selection """
        if self.cfg.kp_selection.local_bestN.enable:
            kp_sel_method = local_best_km
            outputs.update(
                kp_sel_method(
                    kp1=kp1,
                    kp2=kp2,
