...
class KittiEvalOdom():
    ...
    def plot_trajectory(self, poses_gt, poses_result,poses_orig, seq):
        ...
        """
        plot_keys = ["Ground Truth", "Ours", "Baseline"]
        fontsize_ = 20

        poses_dict = {}
        poses_dict["Ground Truth"] = poses_gt
        poses_dict["Ours"] = poses_result
        poses_dict["Baseline"] = poses_orig
    ...
    def eval(self, gt_dir, result_dir, orig_dir,
                alignment=None,
                seqs=None):
        ...
        for i in self.eval_seqs:
            ...
            poses_orig =self.load_poses_from_txt(orig_dir+"/"+file_name)
            ...
            for cnt in poses_result:
                ...
                poses_orig[cnt]= np.linalg.inv(orig_0) @ poses_orig[cnt]
            if alignment == "scale":
                poses_result = self.scale_optimization(poses_gt, poses_result)
                poses_orig = self.scale_optimization(poses_gt, poses_orig)
                ...
                for cnt in poses_result:
                    ...
                    xyz_orig.append([poses_orig[cnt][0, 3], poses_orig[cnt][1, 3], poses_orig[cnt][2, 3]])
                xyz_gt = np.asarray(xyz_gt).transpose(1, 0)
                xyz_result = np.asarray(xyz_result).transpose(1, 0)
                xyz_orig = np.asarray(xyz_orig).transpose(1, 0)

                r, t, scale = umeyama_alignment(xyz_result, xyz_gt, alignment!="6dof")
                ro, to, scaleo = umeyama_alignment(xyz_orig, xyz_gt, alignment!="6dof")
                ...
                align_transformation_o = np.eye(4)
                align_transformation_o[:3:, :3] = ro
                align_transformation_o[:3, 3] = to


                for cnt in poses_result:
                    poses_result[cnt][:3, 3] *= scale
                    poses_orig[cnt][:3, 3] *= scaleo
                    if alignment=="7dof" or alignment=="6dof":
                        poses_result[cnt] = align_transformation @ poses_result[cnt]
                        poses_orig[cnt] = align_transformation_o @ poses_orig[cnt]
            ...
            # Plotting
            self.plot_trajectory(poses_gt, poses_result,poses_orig, i)
